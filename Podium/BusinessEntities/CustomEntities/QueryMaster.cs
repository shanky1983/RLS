using System;
using System.Collections.Generic;
using System.Text;

namespace Attune.Podium.BusinessEntities
{
    public class QueryMaster
    {
        #region Fields
        
        #endregion


        #region Properties

        public string PatientID { get; set; }
        public string PatientVisitID { get; set; }
        public string ViewType { get; set; }
        public string PatientName { get; set; }
        public string SPatientName { get; set; }
        public string DateOfBirth { get; set; }
        public string InPatientNo { get; set; }
        public string CellNo { get; set; }
        public string Purpose { get; set; }
        public string ADMCancelStatus { get; set; }
        public string RateValue { get; set; }
        public string FinalBillID { get; set; }
        public string BillingDate { get; set; }
        public string StatusValue { get; set; }
        public string Servicevalue { get; set; }
        public string SpecialityID { get; set; }
        public string ReferID { get; set; }
        public string ReferringType { get; set; }
        public string Querystring { get; set; }
        public string Role { get; set; }
        public string PatientNumber { get; set; }
        public string RoomNo { get; set; }
        public string IPBPValue { get; set; }
        public string DDLvalue { get; set; }
        public string PatNumber { get; set; }
        public string PageValue { get; set; }
        public string BillNumber { get; set; }
        public string CreditValue { get; set; }
        public string Surgery { get; set; }
        public string BillTypeValue { get; set; }
        public string PODValue { get; set; }
        public string IntendID { get; set; }
        public string LocID { get; set; }
        public string RecvOrgID { get; set; }
        public string IdentityValue { get; set; }
        public string GridViewValue { get; set; }
        public string SupplierValue { get; set; }
        public string QuotationValue { get; set; }
        public string PPatientVisitID { get; set; }
        public string CountValue { get; set; }
        public string SecuredCode { get; set; }
        public string ProcedureID { get; set; }
        public string EmployeeNo { get; set; }
        public string PhysicianID { get; set; }
        public string TaskID { get; set; }
        public string KitIdentity { get; set; }
        public string MasterIdentity { get; set; }
        public string KitBatchNumber { get; set; }
        public string LoginID { get; set; }
        public string PORequestID { get; set; }
        public string PrescriptionNo { get; set; }
        public string actionCode { get; set; }
        public string searchtype { get; set; }
        public string GuId { get; set; }
        public string SampleCollectAgain { get; set; }
        public string SampleID { get; set; }
        public string OrgID { get; set; }
        public string PrintAgain { get; set; }
        public string CategoryCode { get; set; }
        public string InvoiceID { get; set; }
        public string InvoiceNO { get; set; }
        public string ClientID { get; set; }
        public string CName { get; set; }
        public string FrmDate { get; set; }
        public string ToDate { get; set; }
        public string PageID { get; set; }
        public int TaskActionID { get; set; }
        public string AccessionNumber { get; set; }
        public string TestID { get; set; }
        public string Invid { get; set; }
        public string Status { get; set; }
        public string VisitConsID { get; set; }


        #endregion
    }

}
