...
/* Copy Data From Source Template to Intermediate Template Table */		
		...
		Select * into Stage_MHL_T_02_GROUP_MASTER from [Excel_GRP_314] 
		Select * into Stage_MHL_T_01_TESTMASTER from Excel_INV_314 
		Select * into Stage_MHL_T_04_GROUP_TESTS from [Excel_GRP_Content_314]  
		Select * into Stage_MHL_T_03_Package_Master from Excel_PKG_314 
		Select * into Stage_MHL_T_05_PACKAGE_TESTS from [Excel_Card_Content_314] 
		...
		Update Stage_MHL_T_01_TESTMASTER SET PROCESSING_LOCATION='Hosur' where SubCategory='INH'


/* InvestigationMaster */
		Declare @orgid int=314
	    Declare @max Bigint
		Select @max=0	
		Select  @Max = isnull(MAX(InvestigationID),0) From InvestigationMaster
	
		Insert into InvestigationMaster(InvestigationName,Display,InvestigationID,IsParameter)
		
				Select TestName,CASE When stg.Orderable='Y' then 'Y' ELSE 'N' END Display,
				ROW_NUMBER() over (Order By TestName) + @max as InvestigationID,stg.IsParameter
				from Stage_MHL_T_01_TESTMASTER	stg WITH (NOLOCK)
				Left join InvestigationMaster inv WITH (NOLOCK) on  inv.InvestigationName=stg.testname
				Where TestType = 'INV' and Inv.InvestigationName is null
	    ...
	    Update M set InvestigationID=T.InvestigationId
				 ...
				 Inner Join InvestigationMaster T WITH (NOLOCK) on T.InvestigationName=M.testname
...
/* DeptMaster */			
	    ...
	    Declare @max Bigint
          ...
          Insert into InvDeptMaster(DeptName,SequenceNo,Display,OrgID,DeptID)		
			...
			Inner Join DeptMaster M WITH (NOLOCK) on M.DeptName=T.DeptName where T.OrgID=314
...
/* InvestigationID,DeptID,SampleCode,MethodID,SampleContainerID,HeaderId,UomId updation Into Stage_MHL_T_01_TESTMASTER  */
	...
	Update T set InvestigationID=IM.InvestigationID
			 ...
			 Inner Join InvestigationMaster IM WITH (NOLOCK) on T.TestName=IM.InvestigationName
	...
	Insert into InvGroupMaster(GroupName,Type,GroupID,TestCode)
		       Select stg.PACKAGE_NAME,'PKG',ROW_NUMBER() over (order by stg.PACKAGE_NAME) + @max as GroupID,
		              ...
		              From Stage_MHL_T_03_Package_Master Stg WITH (NOLOCK)
		                   left join Invgroupmaster grp WITH (NOLOCK) on  grp.Type='PKG' and stg.PACKAGE_NAME=grp.GroupName
		                   Where  1= 1  and grp.GroupName is null   isnull(stg.METROLAB_PROFILECODE,'') <> '' and grp.Groupid is null
		...
		Insert into InvOrgGroup(AttGroupID,OrgID,DisplayText,IsNonOrderable,Classification,OrgGroupID,TestCode,							 
				...
				select Distinct igm.Groupid,
								...
								ROW_NUMBER() over (order by stg.DISPLAY_NAME) + @max as SequenceNo 
						From Stage_MHL_T_03_Package_Master stg WITH (NOLOCK)
						Inner join InvGroupMaster IGM WITH (NOLOCK) on stg.PACKAGE_NAME=IGM.GroupName
						Left Join InvOrgGroup Invgrp WITH (NOLOCK) on IGM.Groupid = invgrp.Attgroupid and invgrp.OrgID=@orgID
						Where 1 = 1 and invgrp.attgroupid is null and IGM.Type ='PKG'
			
			
			Update T Set T.OrgGroupID=S.OrgGroupID from Stage_MHL_T_03_Package_Master T WITH (NOLOCK)
			         inner join InvGroupMaster I WITH (NOLOCK) ON T.PACKAGE_NAME=I.GroupName 
