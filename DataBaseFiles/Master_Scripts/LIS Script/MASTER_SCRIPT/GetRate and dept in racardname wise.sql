...
Select 
IOM.Investigationid,IMM.investigationname,iom.displaytext,'INV' Type,Idm.deptname ,IRM.Rate ,im.RateName
from   CodeMaster CM 
...
select  *  from (
Select 
IOM.OrgGroupID,IMM.GroupName,iom.displaytext,'GRP' Type,'' deptname ,IRM.Rate ,im.RateName
from   CodeMaster CM 
Inner Join CodeMapper CP On CM.CodeMasterID=CP.CodeMasterID and cm.CodeType=Cp.IdentifyingType
...
Where CM.CodeSchemeID=6 and isnull(iom.IsNonOrderable,'Y')='Y' and 
...
select  *  from (
Select 
IOM.OrgGroupID,IMM.GroupName,iom.displaytext,'PKG' type,'' deptname,IRM.Rate ,im.RateName
from   CodeMaster CM 
Inner Join CodeMapper CP On CM.CodeMasterID=CP.CodeMasterID and cm.CodeType=Cp.IdentifyingType
Inner Join InvOrgGroup IOM ON IOM.OrgGroupID=Cp.IdentifyingID
