...
Begin Try
	Begin Tran
		...
		Declare @ProcessingLocation varchar(50) = 'Bangalore'
...
/* Copy Data From Source Template to Intermediate Template Table */		
		...
		Select * into Stage_MHL_T_02_GROUP_MASTER from Excel_GRP_67 
		Select * into Stage_MHL_T_01_TESTMASTER from Excel_INV_67 
		Select * into Stage_MHL_T_04_GROUP_TESTS from Excel_GRP_Content_67  
		--Select * into Stage_MHL_T_03_Package_Master from [Excel_PKG_Card_67] 
		--Select * into Stage_MHL_T_05_PACKAGE_TESTS from [Excel_PKG_Content_Card_67] 
		...
		Update Stage_MHL_T_01_TESTMASTER SET PROCESSING_LOCATION='Wattala-Hospital' where SubCategory='INH'
...
/* InvestigationMaster */
		...
		Insert into InvestigationMaster(InvestigationName,Display,InvestigationID,TestCode,IsParameter)
				...
				Left join InvestigationMaster inv WITH (NOLOCK) on stg.TCode = inv.TestCode-- and stg.TestName=INV.InvestigationName
				Where TestType = 'INV'
		...
		Update M set InvestigationID=T.InvestigationId
				 ...
				 Inner Join InvestigationMaster T WITH (NOLOCK) on T.TestCode=M.Test_Code and M.TEST_NAME=T.InvestigationName
				 Where M.TYPE = 'INV'			
...
/* DeptMaster */			
	    ...
	    Declare @max Bigint
           ...
           Insert into InvDeptMaster(DeptName,SequenceNo,Display,OrgID,DeptID)		
			...
			Inner Join DeptMaster M WITH (NOLOCK) on M.DeptName=T.DeptName where T.OrgID=67
...
/* Insert INV - Tcode,EdosCode and AbbCode into CodeMAster and CodeMapper */
	...
	Select Stg.Tcode,'Investigations',1,Testname
	From Stage_MHL_T_01_TESTMASTER stg WITH (NOLOCK)
	left join Codemaster CM WITH (NOLOCK) on stg.TCode = Cm.Codename and Cm.Codeschemeid = 1 and stg.testname=cm.codedesc
	...
	From Stage_MHL_T_01_TESTMASTER stg WITH (NOLOCK)
	Inner join Codemaster CM WITH (NOLOCK) on stg.TCode = Cm.Codename and Cm.Codeschemeid = 1  and stg.TestName = CM.CodeDesc
...
/* Insert InvestigationOrgMapping  */
	...
	Insert into InvestigationOrgMapping(orgID,DisplayText,InvestigationID,DeptID,SampleCode,MethodID,SampleContainerID,UOMID,SubCategory,
											...
											IsDiscountable,ReferenceRange,SequenceNo,PanicRange,ProtocalGroupID,Billingname)
...
Declare @orgid int=67
	...
	Select Distinct PatternID,InvestigationID,67
		From
			( 
...
Declare @orgid int=67
	   Declare @max Bigint
 INSERT INTO InvReportMapping (TemplateID,InvestigationID,ORGID)
	Select Distinct TemplateID,InvestigationID,67
		From
			( 
...
Declare @orgid int=67
	    ...
	    Declare @max Bigint
	        ...
	        Insert into InvOrgGroup(AttGroupID,OrgID,DisplayText,IsNonOrderable,Classification,OrgGroupID,TestCode,ValidationText,
										...
										IsSpecialFormat,SequenceNo,ProtocalGroupID,billingname)
				...
				select Distinct igm.Groupid,
								...
								stg.DISPLAY_NAME
						From Stage_MHL_T_02_GROUP_MASTER stg WITH (NOLOCK)
						Inner join InvGroupMaster IGM WITH (NOLOCK) on stg.METRO_COMBINE_CODE = IGM.TestCode and IGM.GroupName=stg.GROUPNAME
			...
			Update T Set T.OrgGroupID=S.OrgGroupID from Stage_MHL_T_02_GROUP_MASTER T WITH (NOLOCK)
			         ...
			         Where S.OrgID =67 and I.Type ='GRP'
...
/* Insert GRP - Tcodes,EdosCode and AbbCode Into CodeMaster and CodeMapper */
	 Declare @orgid int=67
	    Declare @max Bigint
				INSERT INTO CodeMaster(CodeName,CodeType,CodeSchemeID,CodeDesc)
					SELECT METRO_COMBINE_CODE AS CodeName,'GRP',1,GROUPNAME AS CodeDesc 
					FROM Stage_MHL_T_02_GROUP_MASTER stg WITH (NOLOCK)
					left join Codemaster CM WITH (NOLOCK) on Stg.METRO_COMBINE_CODE = CM.Codename and CM.Codeschemeid = 1 and Cm.CodeType = 'GRP'
 				...
 				--	INSERT INTO CodeMaster(CodeName,CodeType,CodeSchemeID,CodeDesc)
					...
					FROM Stage_MHL_T_02_GROUP_MASTER stg WITH (NOLOCK)
					inner join Codemaster CM WITH (NOLOCK) on Stg.METRO_COMBINE_CODE = CM.Codename and CM.Codeschemeid = 1 and Cm.CodeType = 'GRP' 
...
/* Update InvestigationId and OrgGroupId Into Stage_MHL_T_04_GROUP_TESTS */					
 Declare @orgid int=67
      ...
      Update T set OrgGroupID=IOG.OrgGroupId
               ...
               Inner Join CodeMaster CM WITH (NOLOCK) ON CM.CodeName = T.GROUP_CODE and CM.CodeSchemeID =1
      ...
      Update M  set InvestigationID=T.OrgGroupID
		...
		--select * from Stage_MHL_T_04_GROUP_TESTS where group_code='L0026'

		select * from Stage_MHL_T_01_TESTMASTER where tcode in ('L0171'),'L0026_2','L0026_3')
	

	select * from invgroupmapmaster where groupid=126405


	select * from investigationorgmapping where investigationid in (11228,11272,11274)
...
Declare @orgid int=67
      ...
      Insert into InvGroupMapMaster(GroupID,InvestigationID,SequenceNo,Parent,Active)
		Select stg.OrgGroupID,stg.InvestigationID,stg.test_Sequence_No,'N','Y' as Active
			...
			and grp.GroupID is null and stg.orggroupid is not null and stg.investigationid is not null
	...
	--	Insert into InvGroupMaster(GroupName,Type,GroupID,TestCode)
		       Select stg.PACKAGE_NAME,'PKG',ROW_NUMBER() over (order by stg.PACKAGE_NAME) + @max as GroupID,
		              ...
		              From Stage_MHL_T_03_Package_Master Stg WITH (NOLOCK)
		                   left join Invgroupmaster grp WITH (NOLOCK) on stg.METROLAB_PROFILECODE = grp.TestCode AND grp.Type='PKG'-- and grp.groupname=stg.PACKAGE_NAME
	   ...
	   Declare @orgid int=67
		...
		Insert into InvOrgGroup(AttGroupID,OrgID,DisplayText,IsNonOrderable,Classification,OrgGroupID,TestCode,							 
										...
										Gender,groupinterpretation,Issummaryworklist,IsSpecialFormat,SequenceNo,ProtocalGroupID,billingname)
				select Distinct igm.Groupid,
				                @orgID,
				                stg.DISPLAY_NAME,
								CASE WHEN ISNULL(stg.IS_ORDERABLE,'N')='N' THEN 'Y' ELSE 'N' END,
						...
						From Stage_MHL_T_03_Package_Master stg WITH (NOLOCK)
						Inner join InvGroupMaster IGM WITH (NOLOCK) on stg.METROLAB_PROFILECODE = IGM.TestCode --and IGM.GroupName=stg.PACKAGE_NAME
						Left Join InvOrgGroup Invgrp WITH (NOLOCK) on IGM.Groupid = invgrp.Attgroupid and invgrp.OrgID=@orgID
			...
			Update T Set T.OrgGroupID=S.OrgGroupID from Stage_MHL_T_03_Package_Master T WITH (NOLOCK)
			         inner join InvGroupMaster I WITH (NOLOCK) ON T.METROLAB_PROFILECODE = I.TestCode and I.GroupName=T.PACKAGE_NAME
			         inner join InvOrgGroup S WITH (NOLOCK) ON I.GroupID = S.AttGroupID and S.OrgID =67
...
Declare @orgid int=67
	    Declare @max Bigint
				Update T
					...
					Inner Join CodeMaster CM WITH (NOLOCK) ON CM.CodeName = T.PACKAGE_CODE and CM.CodeSchemeID =72
				...
				Update M
					...
					Inner JOIN CodeMaster CM WITH (NOLOCK) ON CM.CodeName = M.TEST_CODE and CM.CodeSchemeID =72
					Inner JOIN CodeMapper CP WITH (NOLOCK) ON CM.CodeMasterID = CP.CodeMasterID and CP.IdentifyingType ='GRP'
					AND CM.CodeType = CP.IdentifyingType 
...
/* Insert PKG Contents */
	  ...
	  Insert into InvPackageMapping(PackageID,ID,SequenceNo,Type,active)
		Select stg.OrgGroupID,stg.InvestigationID,stg.test_Sequence_No,'INV','A' as Active
			...
			and grp.PackageID is null and stg.orggroupid is not null and stg.investigationid is not null
		...
		Select stg.OrgGroupID,stg.InvestigationID,stg.test_Sequence_No,'GRP','A' as Active
			...
			select *  from invpackagemapping where packageid=148539
