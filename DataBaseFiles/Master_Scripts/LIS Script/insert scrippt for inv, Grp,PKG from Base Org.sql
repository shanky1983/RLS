...
/* New INV,GRP,PKG Creation Script */
    select * from organizationaddress where orgid=162
    
Begin Try
	Begin Tran
		Declare @orgID int = 162
...
/* Copy Data From Source Template to Intermediate Template Table */		
		...
		Select * into Stage_MHL_T_02_GROUP_MASTER from Excel_GRP_162 
		Select * into Stage_MHL_T_01_TESTMASTER from Stage2_MHL_T_01_TESTMASTER 
		Select * into Stage_MHL_T_04_GROUP_TESTS from Stage2_MHL_T_04_GROUP_TESTS  
		Select * into Stage_MHL_T_03_Package_Master from Excel_PKG_162 
		Select * into Stage_MHL_T_05_PACKAGE_TESTS from Stage2_MHL_T_05_PACKAGE_TESTS 

		Update Stage_MHL_T_02_GROUP_MASTER set Orggroupid = Null
		--Update Stage_MHL_T_01_TESTMASTER   set InvestigationID = null
		Update Stage_MHL_T_04_GROUP_TESTS SET investigationid=NULL where type='GRP'
		Update Stage_MHL_T_03_Package_Master SET OrgGroupID=NULL
		Update Stage_MHL_T_05_PACKAGE_TESTS SET investigationid=NULL where type='GRP'
...
/* InvestigationMaster */
		...
		Update M set InvestigationID=T.InvestigationId
				 ...
				 Inner Join InvestigationMaster T WITH (NOLOCK) on T.investigationid=M.
				 Where M.TYPE = 'INV'			
...
/* DeptMaster */			
	    ...
	    Declare @max Bigint
           ...
           Insert into InvDeptMaster(DeptName,SequenceNo,Display,OrgID,DeptID)		
			...
			Inner Join DeptMaster M WITH (NOLOCK) on M.DeptName=T.DeptName where T.OrgID=@orgID
...
Declare @orgid int=162
	...
	Select Distinct PatternID,InvestigationID,162
		From
			( 
...
Declare @orgid int=162
 ...
 INSERT INTO InvReportMapping (TemplateID,InvestigationID,ORGID)
	Select Distinct TemplateID,InvestigationID,162
		From
			( 
	    ...
	    Declare @max Bigint
	        ...
	        Insert into InvOrgGroup(AttGroupID,OrgID,DisplayText,IsNonOrderable,Classification,OrgGroupID,TestCode,ValidationText,
				...
				select Distinct igm.Groupid,
								...
								stg.billing_name
						From Stage_MHL_T_02_GROUP_MASTER stg WITH (NOLOCK)
						Inner join InvGroupMaster IGM WITH (NOLOCK) on stg.attgroupid=IGM.groupid
			...
			Update T Set T.OrgGroupID=S.OrgGroupID 
	                 from Stage_MHL_T_02_GROUP_MASTER T WITH (NOLOCK)
			         Inner JOIN InvGroupMaster I WITH (NOLOCK) ON T.attgroupid=I.groupid
			         Inner JOIN InvOrgGroup S WITH (NOLOCK) ON S.AttGroupID = I.GroupID 
			         Where S.OrgID =162 and I.Type ='GRP'
...
/* Insert GRP - Tcodes,EdosCode and AbbCode Into CodeMaster and CodeMapper */
	 Declare @orgid int=162
	    Declare @max Bigint
			--	INSERT INTO CodeMaster(CodeName,CodeType,CodeSchemeID,CodeDesc)
					SELECT METRO_COMBINE_CODE AS CodeName,'GRP',120,GROUPNAME AS CodeDesc 
					FROM Stage_MHL_T_02_GROUP_MASTER stg WITH (NOLOCK)
					Left join Codemaster CM WITH (NOLOCK) on Stg.METRO_COMBINE_CODE = CM.Codename and CM.Codeschemeid = 120 and Cm.CodeType = 'GRP'
 				...
 				--	INSERT INTO CodeMaster(CodeName,CodeType,CodeSchemeID,CodeDesc)
					...
					FROM Stage_MHL_T_02_GROUP_MASTER stg WITH (NOLOCK)
					inner join Codemaster CM WITH (NOLOCK) on Stg.METRO_COMBINE_CODE = CM.Codename and CM.Codeschemeid = 120 and Cm.CodeType = 'GRP' 
...
/* Update InvestigationId and OrgGroupId Into Stage_MHL_T_04_GROUP_TESTS */					
 Declare @orgid int=162
      ...
      Update T set OrgGroupID=IOG.OrgGroupId
               ...
               Inner Join CodeMaster CM WITH (NOLOCK) ON CM.CodeName = T.GROUP_CODE and CM.CodeSchemeID =120
      ...
      Update M  set InvestigationID=T.OrgGroupID
				From Stage_MHL_T_04_GROUP_TESTS M WITH (NOLOCK)
				Inner Join CodeMaster CM WITH (NOLOCK) ON CM.CodeName = M.TEST_CODE and CM.CodeSchemeID =120
                Inner Join CodeMapper CP WITH (NOLOCK) ON CM.CodeMasterID = CP.CodeMasterID and CP.IdentifyingType ='GRP'
                AND CM.CodeType = CP.IdentifyingType 
	...
	--select * from Stage_MHL_T_04_GROUP_TESTS
	   ...
	   Declare @orgid int=162
		...
		Insert into InvOrgGroup(AttGroupID,OrgID,DisplayText,IsNonOrderable,Classification,OrgGroupID,TestCode,							 
			...
			Update T Set T.OrgGroupID=S.OrgGroupID 
			    from Stage_MHL_T_03_Package_Master T WITH (NOLOCK)
			         inner join InvGroupMaster I WITH (NOLOCK) ON T.Attgroupid=I.Groupid
			         inner join InvOrgGroup S WITH (NOLOCK) ON I.GroupID = S.AttGroupID and S.OrgID =162
...
/* Insert PKG - Tcodes,EdosCode and AbbCode Into CodeMaster and CodeMapper */ 
	 ...
	 Declare @orgid int=162
	    Declare @max Bigint
					--INSERT INTO CodeMaster(CodeName,CodeType,CodeSchemeID,CodeDesc)
					SELECT METROLAB_PROFILECODE AS CodeName,'PKG',120,PACKAGE_NAME AS CodeDesc 
					FROM Stage_MHL_T_03_Package_Master stg WITH (NOLOCK)
					Left join Codemaster CM WITH (NOLOCK) on Stg.METROLAB_PROFILECODE = CM.Codename and CM.Codeschemeid = 120 and Cm.CodeType = 'PKG'
 				...
 				--	INSERT INTO CodeMaster(CodeName,CodeType,CodeSchemeID,CodeDesc)
					...
					FROM Stage_MHL_T_03_Package_Master stg WITH (NOLOCK)
					inner join Codemaster CM WITH (NOLOCK) on Stg.METROLAB_PROFILECODE = CM.Codename and CM.Codeschemeid = 120 and Cm.CodeType = 'PKG' 
...
Declare @orgid int=162
	    Declare @max Bigint
				Update T
					...
					Inner Join CodeMaster CM WITH (NOLOCK) ON CM.CodeName = T.PACKAGE_CODE and CM.CodeSchemeID =120
				...
				--	From Stage_MHL_T_05_PACKAGE_TESTS M WITH (NOLOCK)
				--	Inner JOIN CodeMaster CM WITH (NOLOCK) ON CM.CodeName = M.TEST_CODE and CM.CodeSchemeID = 10
