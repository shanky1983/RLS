...
function VatTblist()
{
 ...
 try {
    ...
    var Act = SListForAppDisplay.Get("StockOutFlow_Scripts_StockUsage_js_06ss") == null ? "Action" : SListForAppDisplay.Get("StockOutFlow_Scripts_StockUsage_js_06");
    var GrandTot = SListForAppDisplay.Get("StockOutFlow_Scripts_StockUsage_js_06ss") == null ? "Grand Total" : SListForAppDisplay.Get("StockOutFlow_Scripts_StockUsage_js_06");
    var Vat = SListForAppDisplay.Get("StockOutFlow_Scripts_StockUsage_js_044www") == null ? "VAT (%)" : SListForAppDisplay.Get("StockOutFlow_Scripts_StockUsage_js_0044");
    
    var Tbl_Th=$('<tr/>');
    Tbl_Th.append($('<th/>').html(ProName)).append($('<th/>').html(Qty)).append($('<th/>').html(SellUnits)).append($('<th/>').html(UCost)).append($('<th/>').html(Tot)).append($('<th/>').html(Dis)).append($('<th/>').html(CompliQty)).append($('<th/>').html(Vat)).append($('<th/>').html(TotCost)).append($('<th/>').html(SupplierLis)).append($('<th/>').html(Act));
    //;
    $('[id$="tblOrederedItems"]').append(Tbl_Th);
    var Tbl_Tr;
    ...
    $.each(lstProductList, function (obj, value) {
     ...
     var ProName = '<a  id="tdProDiv"  class="lnkBtn pointer" title="Click to view" href="" onclick="return showLocationdetails(' + value.ProductID + ",'" + value.ProductName + "');\" >" +  unescape(value.ProductName) + "</a>";
     var Cell_Action = "<input name='Edit' onclick='btnEdit_OnClick(" + JSON.stringify(value) + ");' value = '' type='button' class='ui-icon ui-icon-pencil pull-left'  /> " +
                                "<input name='Delete' onclick='btnDelete(" + JSON.stringify(value) + ");' value = '' type='button' class='ui-icon ui-icon-trash pull-left' />"
     
     Tbl_Tr.append($('<td/>').html(ProName)).append($('<td/>').html(value.Quantity).css('text-align', 'right')).append($('<td/>').html(value.Unit)).append($('<td/>').html(value.UnitCostPrice).css('text-align', 'right')).append($('<td/>').html(parseFloat(value.Total).toFixed(2)).css('text-align', 'right')).append($('<td/>').html(parseFloat(value.Discount).toFixed(2)).css('text-align', 'right')).append($('<td/>').html(parseFloat(value.ComplimentQTY).toFixed(2)).css('text-align', 'right')).append($('<td/>').html(value.Tax).css('text-align', 'right')).append($('<td/>').html(parseFloat(value.Rate).toFixed(2)).css('text-align', 'right')).append($('<td/>').html("<a class='lnkBtn pointer' href='' onclick='return SupplierListPopUp(" + value.ProductID + ");' >" + 'Supplierlist' + "</a>")).append($('<td/>').html(Cell_Action));

     
     $('[id$="tblOrederedItems"]').append(Tbl_Tr);
    });
    
    var tbl_foot =$('<tr/>').append($('<td/>').html("Grand Total").attr('colspan',8).css('text-align', 'right')).append($('<td/>').html(parseFloat(tGrandTotal).toFixed(2)).css('text-align', 'right')).append($('<td/>').html("").attr('colspan',2));
    tbl_foot.addClass("gridHeader");
    
    
    $('[id$="tblOrederedItems"]').append(tbl_foot);
