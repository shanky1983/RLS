...
function clearbuttonClick() {
    ...
    if (window.confirm(objClear)) {
        clearPageControlsValue('N');
          if ($('#hdnPageType').val() == "B2Bill") 
                            {
                       document.getElementById('ddlSex').value = "0";
                        document.getElementById('rblSearchType_0').checked = true;
                        document.getElementById('rblSearchType_1').checked = false;
                        document.getElementById('rblSearchType_2').checked = false;
                        document.getElementById('rblSearchType_3').checked = false;
                        document.getElementById('rblSearchType_4').checked = false;
                         document.getElementById('rblSearchType_5').checked = false;
...
function SetDiscountAmt() {
 //arun changes - when coupon selected, we apply discont means should clear rhe payment details
    var ctrlPaymentType = document.getElementById('billPart_PaymentType_ddlPaymentType');
    ...
    if (sPaymentTypeId == "10" && pDiscountPercent.selectedIndex > 0) {
      //  document.getElementById('billPart_PaymentType_dvPaymentTable').innerHTML = "";
        ClearPaymentControlEvents1();
        var net = document.getElementById('billPart_txtNetAmount').value;
        document.getElementById('billPart_txtDue').value = format_number(net, 2);
        document.getElementById('billPart_PaymentType_ddlPaymentType').value = "1~N~100~100";
        document.getElementById('billPart_PaymentType_txtAmount').value = format_number(net, 2);
        //document.getElementById('billPart_PaymentType_txtTotalAmount').value = format_number(net, 2)
        document.getElementById('billPart_PaymentType_txtTotalAmount').innerHTML = format_number(net, 2);
        ToTargetFormat($('#billPart_PaymentType_txtTotalAmount'));
        //coupon name search box hide
        document.getElementById('billPart_PaymentType_Rs_Bank').style.display = 'none';
        document.getElementById('billPart_PaymentType_txtBankType').style.display = 'none';
        //coupn number box hide
        document.getElementById('billPart_PaymentType_Rs_Cheque_no').style.display = 'none';
        document.getElementById('billPart_PaymentType_txtNumber').style.display = 'none';
        //units hide
        document.getElementById('billPart_PaymentType_lblUnits').style.display = 'none';
        document.getElementById('billPart_PaymentType_tdtxtUnits').style.display = 'none';               
    }
   //
...
function ClientSelected(source, eventArgs) {
    ...
    // Seetha Item Level Discount //

    if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y") {

        if (ClientCorpCode != 'GENERAL') {

        document.getElementById('billPart_ddlitemdiscountType').disabled = true;
        document.getElementById('billPart_txtdiscountvalue').disabled = true;
        document.getElementById('billPart_textdiscountAmount').disabled = true;
        document.getElementById('billPart_ddlitemdiscountReson').disabled = true;

        }
        else {
            document.getElementById('billPart_ddlitemdiscountType').disabled = false;
           // document.getElementById('billPart_txtdiscountvalue').disabled = false;
           // document.getElementById('billPart_textdiscountAmount').disabled = false;
            document.getElementById('billPart_ddlitemdiscountReson').disabled = false;
        }
    }
    //Seetha Item Level Discount //
}
...
function BillingItemSelected(source, eventArgs) {
    ...
    if (list.length > 0) {
        if (list[i] != "") {
            ...
            outRInSourceLocation = list[5];
            //Seetha Item level discount //
            if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y" && $("#hdnPageType").val() != "B2B") {
            var clientrate = list[8];
            var MRPRate = list[9];
            var MappedRate = list[10].trim();
            var MRP = list[11].trim();
            var Isdiscountable = list[12].trim();
            var ClientCode = list[13].trim();
            var CorrectClientCode = 'General';


                if (MappedRate == 'Y' && MRP == "Y") {
                    document.getElementById('billPart_txtclientrate').value = clientrate;
                    document.getElementById('billPart_txtMRP').value = MRPRate;
                }

            if (MappedRate == 'Y' && MRP == "N") {
                document.getElementById('billPart_txtclientrate').value = clientrate;
                document.getElementById('billPart_txtMRP').value = "0.00";
                
            }
            if (MappedRate == 'N' && MRP == "Y") {
                document.getElementById('billPart_txtMRP').value = MRPRate;
                document.getElementById('billPart_txtclientrate').value = "0.00";
            }

            if (MappedRate == 'N' && MRP == "N") {
                document.getElementById('billPart_txtclientrate').value = "0.00";
                document.getElementById('billPart_txtMRP').value = "0.00";
            }
            ...
            if (Isdiscountable == 'N') {
                ...
                if (ClientCode.toUpperCase() != CorrectClientCode.toUpperCase()) {
                    document.getElementById('billPart_ddlitemdiscountType').disabled = true;
                    document.getElementById('billPart_txtdiscountvalue').disabled = true;
                    document.getElementById('billPart_textdiscountAmount').disabled = true;
                    document.getElementById('billPart_ddlitemdiscountReson').disabled = true;
...
function CmdAddBillItemsType_onclick(FeeID, FeeType, Descrip, Amount, Remarks, IsRI, ReportDate, ActualAmount, IsNormalRateCard, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID, DiscountPolicyID, DiscountCategoryCode, ReportDeliveryDate, MaxDiscount, IsRedeem, RedeemAmount,IsSpecialTest, Ishtmltab, IsTemplateID) {
    ...
    var MRPAmount = 0;
    var DiscountValue = 0;
    var pDiscountPercent = "";
    var DiscountPercent = "";
    var SDiscountValue = "";
    var ItemDisPercent = 0;
    var pDiscountReason = "";
    var DiscountReason = "";
    DiscountValue = parseFloat(DiscountValue).toFixed(2);
    if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y" && $("#hdnPageType").val() != "B2B") {

        clientAmount = document.getElementById('billPart_txtclientrate').value;

         MRPAmount = document.getElementById('billPart_txtMRP').value;

        DiscountValue = document.getElementById('billPart_txtdiscountvalue').value;

        DiscountValue = parseFloat(DiscountValue).toFixed(2);



         pDiscountPercent = document.getElementById('billPart_ddlitemdiscountType');
        DiscountPercent = pDiscountPercent.options[pDiscountPercent.selectedIndex].value;
         SDiscountValue = DiscountPercent.split('~');
        ItemDisPercent = 0;
        if (SDiscountValue[0] > 0) {
            ItemDisPercent = SDiscountValue[0];
        }

        var pDiscountReason = document.getElementById('billPart_ddlitemdiscountReson');
        var DiscountReason = pDiscountReason.options[pDiscountReason.selectedIndex].innerText;
    ...
    if (document.getElementById('txtClient').value.trim() == '' && document.getElementById('hdnDefaultOrgBillingItems').value != '' && defaultbillflag == 0) {
        ...
        if (itemleveldiscountneed == "Y") {
            ItemAmount = (defalutdata[3] - DiscountValue).toFixed(2);
        }
        if (itemleveldiscountneed == "Y") {
            FeeViewStateValue = "FeeID^" + defalutdata[0] + "~FeeType^" + defalutdata[1] + "~Descrip^" + defalutdata[2] + "~Amount^"
                + ItemAmount + "~Quantity^" + defalutdata[4] + "~Remarks^" + defalutdata[5] + "~ReportDate^" + defalutdata[6] + "~IsReimbursable^" + defalutdata[7] +
				...
				"~BaseRateID^" + 0 + "~DiscountPolicyID^" + 0 + "~DiscountCategoryCode^" + '' + "~ReportDeliveryDate^" + '' + "~MaxDiscount^" + ''
				 + '' + "~IsRedeem^" + '' + "~RedeemAmount^" + '' + "~IsSpecialTest^" + IsSpecialTest + "~Ishtmltab^" + Ishtmltab + "~IsTemplateID^" + IsTemplateID + "~IsMandatoryHis^" + IsMandatoryHis + "~ClientRate^" + clientAmount + "~MRPAmount^" + MRPAmount + "~DiscountValue^" + DiscountValue + "~ItemDiscountPercentage^" + ItemDisPercent + "~DiscountReason^" + DiscountReason + "~ClientValue^" + clientAmount + "~MRPValue^" + MRPAmount + "|";
    ...
    var itemleveldiscountneed = document.getElementById('billPart_hdnitemleveldiscountneed').value;
    if (itemleveldiscountneed == "Y") {
        Amount = (Amount - DiscountValue).toFixed(2);
    }
...
var itemleveldiscountneed = document.getElementById('billPart_hdnitemleveldiscountneed').value;
var itemleveldicountstyle = 'display:none';
if (itemleveldiscountneed == "Y" && $("#hdnPageType").val() == "B2C") {
    itemleveldicountstyle = 'display:table-cell';
}
//Item Level Discount //
    ...
    // Item level Discount//
    var ClientRate = "C.Rate";
    var MRP = "MRP";
    var DiscountAmount = "D.Value";
    // Item level Discount//
    ...
    + "<th scope='col' style='width:5%;'> " + vS + " </th> <th scope='col' style='width:6%;'> " + vCode + " </th> <th scope='col' align='left' style='width:35%;padding-left:2px;'> " + vDescription + " </th>  <th scope='col' style='" + testhistorystyle + "'> " + Ishistory + " </th> <th scope='col' align='center' style='width:8%;'>" + vIsSTAT + "</th><th scope='col' align='center' style='width:12%;'>" + vPL + "</th> <th scope='col' style='" + itemleveldicountstyle + "'> " + ClientRate + " </th> <th scope='col' style='" + itemleveldicountstyle + "'> " + MRP + " </th> <th scope='col' style='" + itemleveldicountstyle + "'> " + DiscountAmount + " </th> <th scope='col' align='right' style='display:none;width:5%;'>  " + vQuantity + " </th><th scope='col' align='right' style='width:8%;'> " + vAmount + " </th> " 
        ...
        for (iMain = 0; iMain < arrayMainData.length - 1; iMain++) {
            ...
            for (iChild = 0; iChild < arraySubData.length; iChild++) {
                ...
                if (arrayChildData.length > 0) {
                    ...
                    if (arrayChildData[0] == "ClientRate") {
                        ClientRate = arrayChildData[1];
                    }
                    if (arrayChildData[0] == "MRPAmount") {
                        MRPItemAmount = arrayChildData[1];
                    }
                    if (arrayChildData[0] == "DiscountValue") {
                        ItemDiscountValue = arrayChildData[1];

                        ItemDiscountValue = parseFloat(ItemDiscountValue).toFixed(2);
                    }
                    if (arrayChildData[0] == "ItemDiscountPercentage") {
                        ItemDisPercent = arrayChildData[1];
                    }
                    if (arrayChildData[0] == "DiscountReason") {
                        DiscountReason = arrayChildData[1];
                    }
            ...
            ItemDiscountValue = parseFloat(ItemDiscountValue).toFixed(2);
             ...
             newPaymentTables += "<TD align='center' style='" + itemleveldicountstyle + "'>" + ClientRate+ "</TD>";

             newPaymentTables += "<TD align='center' style='" + itemleveldicountstyle + "'>" + MRPItemAmount + "</TD>"

             newPaymentTables += "<TD align='center' style='" + itemleveldicountstyle + "'>" + ItemDiscountValue + "</TD>"; ;
...
function SetNetValue(obj) {
    ...
    if (Number(document.getElementById('billPart_txtGross').value) > 0) {
        if (Number(document.getElementById('billPart_hdnDiscountableTestTotal').value) > 0) {
            ...
            else if (document.getElementById('billPart_hdnNotallowDiscashClient').value == 'Y' && $('#hdnSelectedClientCode').val() == 'GENERAL')
             {
                document.getElementById('billPart_ddDiscountPercent').disabled = false;
            }
            else {
...
function ClearSelectedData() {
    ...
    if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y" && $("#hdnPageType").val() != "B2B") {
        document.getElementById('billPart_txtclientrate').value = "0.00";
        document.getElementById('billPart_txtMRP').value = "0.00";
        document.getElementById('billPart_txtdiscountvalue').value = "0.00";
        document.getElementById('billPart_textdiscountAmount').value = "0.00";
        document.getElementById('billPart_ddlitemdiscountType').selectedIndex = "0";

        document.getElementById('billPart_ddlitemdiscountReson').selectedIndex = "0";
    }
...
function ClearAutocomp() {
    ...
    if(document.getElementById('billPart_txtTestName').value == '')
...
function GetTestHistoryforPatient() {
 ...
 for (i = 0; i < Items.length; i++) {
    ...
    // VEL | Rolling Amount | 10-01-2020 | END  | //
    // Seetha Item Level Discount -- Start //
    function SetItemDiscountAmt() {

        var DiscountType = "";
        var pDiscountPercent = document.getElementById('billPart_ddlitemdiscountType');
        var DiscountPercent = pDiscountPercent.options[pDiscountPercent.selectedIndex].value;
        var SDiscountValue = DiscountPercent.split('~');
        var ItemDisPercent = SDiscountValue[0];
        var DiscountAmount = (parseFloat(document.getElementById('billPart_txtMRP').value) * parseFloat(ItemDisPercent)) / 100;
        
        document.getElementById('billPart_txtdiscountvalue').value = DiscountAmount;
        document.getElementById('billPart_textdiscountAmount').value = (parseFloat(document.getElementById('billPart_txtMRP').value))- DiscountAmount;
        
        
    }
  function checkDiscountValue(id) {
      if (document.getElementById(id).value == '') {
     ...
     // Seetha Item Level Discount -- End //