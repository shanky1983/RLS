                                    ...
                                    $.each(Items, function(index, Item) {
                                        ...
                                        for (var k = 0; k < Items.length; k++) {
                                            ...
                                            if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y") {
                                                Clientrate = defalutdata[28];
                                                MRP = defalutdata[29];

                                                if (defalutdata[30].trim() == "Y") {
                                                    ClientValue = defalutdata[28];
                                                ...
                                                if (defalutdata[31].trim() == "Y") {


                                                    MRPValue = defalutdata[29];
                                                }
                                                else {
                                                    MRPValue = "0.00";
                                                }
                                            ...
                                            if (iPaymentAlreadyPresent == 0) {
                                                ...
                                                if (IsOutSource == 'N') {
                                                    if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y") {
                                                        ...
                                                        "~IsSpecialTest^" + Item.IsSpecialTest + "~Ishtmltab^" + Item.Ishtml + "~IsTemplateID^" + Item.TemplateID + "~IsMandatoryHis^" + IsMandatoryHis + "~ClientRate^" + Clientrate + "~MRPAmount^" + MRP + "~DiscountValue^" + ItemDiscountValue + "~ItemDiscountPercentage^" + ItemDisPercent + "~DiscountReason^" + DiscountReason + "~ClientValue^" + ClientValue + "~MRPValue^" + MRPValue + "|";
                                                    }
                                                    else {
                                                        FeeViewStateValue = "FeeID^" + defalutdata[0] + "~FeeType^" + defalutdata[2] + "~Descrip^" + defalutdata[1] +
                                                        "~Amount^" + defalutdata[3] + "~Quantity^" + 1 + "~Remarks^" + defalutdata[5] + "~ReportDate^" + defalutdata[7] +
                                                    ...
                                                    if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y") {
                                                        ...
                                                        if ($('#hdnPageType').val() == "B2F") {

                                                            FeeViewStateValue = "FeeID^" + defalutdata[0] + "~FeeType^" + defalutdata[2] + "~Descrip^" + defalutdata[1] +
                                                        "~Amount^" + MRP + "~Quantity^" + 1 + "~Remarks^" + defalutdata[5] + "~ReportDate^" + defalutdata[7] +
                                                        "~IsReimbursable^" + defalutdata[6] + "~ActualAmount^" + defalutdata[8] + "~IsNormalRateCard^" + defalutdata[9] +
                                                        "~IsDiscountable^" + defalutdata[9] + "~IsTaxable^" + defalutdata[10] + "~IsRepeatable^" + defalutdata[11] +
...
function SelectedClientPatientForDoFromVisit(source, eventArgs) {
   ...
   var isclientdetails = vlstOutput[51];
   
   var IsClientSubDetails = isclientdetails.split('^');
   var ISCashClient = IsClientSubDetails[13];
...
//
 ...
 if ($('#hdnPageType').val() != "B2F") {
     ...
     document.getElementById('hdnPassportNo').value = Passportno;
}
...
// Seetha Changes for Passport No Validation -- Start //

function CheckExistingPassportNo(ctl) {

    if (document.getElementById('hdnPassport').value == 'Y') {

        var str = document.getElementById('txtpassportno').value;
        var n = str.length;
        if (n > 9 && document.getElementById('txtpassportno').value != '') {
            alert('Kindly Provide Valid PassportNumber!');
    ...
    if (document.getElementById('txtpassportno').value != '') {
        WebService.GetPassportNo(document.getElementById('txtpassportno').value, GetPassportNo1);
...
function GetPassportNo1(URnList) {
    if (URnList.length > 0) {
        var CurrPatientNumber = document.getElementById('hdnPatientNumber').value;

        if (URnList[0].Displaytext != CurrPatientNumber) {
            alert('Passport Number is already assosicate with another Patient..!');
            document.getElementById('txtpassportno').value = "";
            document.getElementById('txtpassportno').focus();
            return false;
        }
...
// Seetha Changes for Passport No Validation -- End //
...
function validateEvents(obj) {
  ...
  //  var vDiscountReason = SListForAppMsg.Get('Billing_Scripts_LabQuickBilling_js_37') == null ? "Provide discount reason" : SListForAppMsg.Get('Billing_Scripts_LabQuickBilling_js_37');
    ...
    var objRefphyNeed = SListForAppMsg.Get('Scripts_LabQuickBilling_js_09') == null ? "Provide Physician Name" : SListForAppMsg.Get('Scripts_LabQuickBilling_js_09');
...
//            document.getElementById('txtPhleboName').focus();
    ...
    if ($('#hdnRefPhyNeed').val() == 'Y') {
        if (document.getElementById('txtInternalExternalPhysician').value == '') {
            ValidationWindowResponse(objRefphyNeed, AlertType);
...
//                document.getElementById('txtPincode').focus();
        ...
        var hdnClientAmount = $('[id$="hdnClientAmountFranchisee"]').val();
        var virtualVal = 0;
                ...
                if (hdnAdvanceClient.trim() == 1) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if ($('#hdnPageType').val() == "B2F") {
                                if ((parseInt(TotalAvailablebalance) < parseInt(hdnClientAmount)) || ((parseInt(TotalAvailablebalance) - parseInt(hdnClientAmount)) < parseInt(hdnThresholdValue3))) {
                                ...
                                else if ((parseInt(TotalAvailablebalance) - parseInt(hdnClientAmount)) < parseInt(hdnThresholdValue2)) {
                                    //alert('Second Alert:Threshold value exceed');
                                    ValidationWindow(UsrAlrtMsg13, AlrtWinHdr);
                                    return false;
                                }
                                else if ((parseInt(TotalAvailablebalance) - parseInt(hdnClientAmount)) < parseInt(hdnThresholdValue)) {
                                    // alert('First Alert: Threshold value exceed');
                                    ValidationWindow(UsrAlrtMsg14, AlrtWinHdr);
                                    return false;
                                }
