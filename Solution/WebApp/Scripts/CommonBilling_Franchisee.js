            ...
            if (data.d.length > 0) {
                for (var i = 0; i < data.d.length; i++) {
                    ...
                    if (arrGotValue.length > 0) {
                        ...
                        var clientAmount = document.getElementById('billPart_hdnClientRateFRA').value;
                        var MRPAmount = document.getElementById('billPart_hdnMRPRateFRA').value;

                        if (clientAmount <= 0) {
                            clientAmount = MRPAmount;
                        }
                       
                        if (parseFloat(MRPAmount) < parseFloat(clientAmount) && document.getElementById('hdnAllowGreaterAmtfromMrp').value == 'N') {
                            alert("Test rate Should not be greater than General rate");
                            return false;
                        }

                        if (document.getElementById('hdnAdvanceClienthavedeposit').value == "N") {
                            alert("Client Deposit balance Amount is Zero");
                            return false;
                        }
    ...
    if (list.length > 0) {
        if (list[i] != "") {
            ...
            if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y" && $("#hdnPageType").val() != "B2B") {
                ...
                document.getElementById('billPart_hdnClientRateFRA').value = clientrate;

                document.getElementById('billPart_hdnMRPRateFRA').value = MRPRate;
...
function uploadtrfvalidation() {
   ...
   // debugger;
    if (trfuploadlist == "avail" || found == true) {
        return true;
    }
    else {
...
function CmdAddBillItemsType_onclick(FeeID, FeeType, Descrip, Amount, Remarks, IsRI, ReportDate, ActualAmount, IsNormalRateCard, IsDiscountable, IsTaxable, IsRepeatable, IsSTAT, IsSMS, IsOutSource, IsNABL, BillingItemRateID, Code, HasHistory, outRInSourceLocation, BaseRateID, DiscountPolicyID, DiscountCategoryCode, ReportDeliveryDate, MaxDiscount, IsRedeem, RedeemAmount,IsSpecialTest, Ishtmltab, IsTemplateID) {
    ...
    if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y" && $("#hdnPageType").val() != "B2B") {
       ...
       clientAmount = document.getElementById('billPart_hdnClientRateFRA').value
       MRPAmount = document.getElementById('billPart_hdnMRPRateFRA').value
        if (clientAmount <= 0) {
            clientAmount = MRPAmount;
        }


       var ClientValue  = document.getElementById('billPart_txtclientrate').value;
       var MRPValue =document.getElementById('billPart_txtMRP').value;
    ...
    if (itemleveldiscountneed == "Y") {
        Amount = (MRPAmount - DiscountValue).toFixed(2);
        ActualAmount = (ActualAmount - DiscountValue).toFixed(2);
    }
...
if (itemleveldiscountneed == "Y" && ($("#hdnPageType").val() == "B2C" || $("#hdnPageType").val() == "B2F")) {
    ...
    if (arrayMainData.length > 0) {
        for (iMain = 0; iMain < arrayMainData.length - 1; iMain++) {
            ...
            for (iChild = 0; iChild < arraySubData.length; iChild++) {
                ...
                if (arrayChildData.length > 0) {
                    ...
                    if (arrayChildData[0] == "ClientRate") {
                        ...
                        ClientAmount = Number(ClientAmount) + Number(ClientRate);
                    }
                    ...
                    if (arrayChildData[0] == "ClientValue") {
                        ClientValue = arrayChildData[1];
                    }
                    if (arrayChildData[0] == "MRPValue") {
                        MRPValue = arrayChildData[1];
                    }
            ...
            ItemDiscountValue = parseFloat(ItemDiscountValue).toFixed(2);
             ...
             newPaymentTables += "<TD align='center' style='" + itemleveldicountstyle + "'>" + ClientValue+ "</TD>";

             newPaymentTables += "<TD align='center' style='" + itemleveldicountstyle + "'>" + MRPValue + "</TD>"
...
//             else {
    ...
    // Franchisee Module Threshold Change //

    document.getElementById('hdnClientAmountFranchisee').value = ClientAmount;
    ToTargetFormat($("#hdnClientAmountFranchisee"));
...
function ClearSelectedData() {
    ...
    if (document.getElementById('billPart_hdnitemleveldiscountneed').value == "Y" && $("#hdnPageType").val() != "B2B") {
        ...
        document.getElementById('billPart_txtMRP').value = "0.00";
        document.getElementById('billPart_hdnClientRateFRA').value = "0.00";

        document.getElementById('billPart_hdnMRPRateFRA').value = "0.00";
...
function GetTestHistoryforPatient() {
 ...
 for (i = 0; i < Items.length; i++) {
    ...
    function SetItemDiscountAmt() {
        ...
        var DiscountAmount = (parseFloat(document.getElementById('billPart_hdnMRPRateFRA').value) * parseFloat(ItemDisPercent)) / 100;
        
        document.getElementById('billPart_txtdiscountvalue').value = DiscountAmount;
        document.getElementById('billPart_textdiscountAmount').value = (parseFloat(document.getElementById('billPart_hdnMRPRateFRA').value)) - DiscountAmount;
