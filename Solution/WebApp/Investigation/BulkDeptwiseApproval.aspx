<%@ Page Language="C#" AutoEventWireup="true" CodeFile="BulkDeptwiseApproval.aspx.cs" Inherits="Investigation_BulkDeptwiseApproval"
...
//               document.getElementById("DivSearchArea").style.diplay="block";
          ...
          $("#tdlblWorkListID").show();
           $("#tdchkIsMaster").show();
            ...
            else if (hdnActionName.value == 'BulkApprove'){
...
//               document.getElementById("DivSearchArea").style.diplay="block";
        ...
        function onAutoSave() {
            try {
                ...
                var DeptId=document.getElementById('hdnDept').value;
                var lstInvValues = [];
                var lstPatInv = [];
                var lstOfInvValues = [];
                var controlID = '';
...
// });
                        ...
                        case 45:
                            ...
                            if(emptyvalcount==1)
                            {
                              alert ("Some Investigation values are empty");
                            }
//                var deptname=$( "#ddlDept option:selected" ).text();
//                Showdoctorslist(OrgID,deptname,roleid);
//                 $find("ModalPopupExtenderDr").show();
...
//});
  else
  {
            ...
            try {
               ...
               if(isappr==true)
               {
               var ddldoctor = $("#ddldoctor :selected").val();
               if(ddldoctor=='--Select--')
               {
               alert('select doctor to approve');
               return false;
               }
               }
...
//        }
        ...
        function BatchValidation() {
         $("#Button3").click();
            /* Added By Venkatesh S */
             if (document.getElementById("hdnActionName").value == "BulkApprove") {
            var AlertType = SListForAppMsg.Get('Investigation_Header_Alert') == null ? "Alert" : SListForAppMsg.Get('Investigation_Header_Alert');
            ...
            try {
                ...
                var ddlHeader = $("#ddlHeader :selected").val();
                 var restype = $("#ddlResultType :selected").val();
                var ddlProtocol = $("#ddlProtocol :selected").val();
                if (ddlProtocol == '0' && $('#txtPatientID').val() == "" && ddlHeader == '0' && restype=='0' && ddlDept == '0' && ddlDevice == '0' && ($('#txtFromVisitID').val() == "" || $('#txtFromVisitID').val() == "0") && ($('#txtToVisitID').val() == "" || $('#txtToVisitID').val() == "0") && ($('#txtWorkListID').val() == "" || $('#txtWorkListID').val() == "0") && ($('#txtInvestigationName').val() == "") && ($('#txtSampleID').val() == "" || $('#txtSampleID').val() == "0")) {
...
//                var ddlProtocol = $("#ddlProtocol :selected").val();
        ...
        function CloseModalPopup() {       
            $("#ModalPopupExtenderDr").modal('hide');
    }
    ...
    <div class="contentdata">
        <asp:UpdatePanel ID="UpdatePanel1" runat="server">
            <ContentTemplate>
                                    ...
                                    <div id="DivSearchArea" class="filterdataheader2" style="display: block;">
                                        <table class="w-100p">
                                            ...
                                            <tr id="trApp">
                                             ...
                                             <tr id="trUser"">
                                                  <td class="w-10p a-right" id="tdlblcolllocation" runat="server">
                                            ...
                                            <tr>
                                              <td class="a-right"><asp:Label ID="lblteststatus" Text="Staus" runat="server"></asp:Label></td>
                                              <td colspan="7">
                                               <asp:DropDownList ID="ddlteststatus" runat="server" CssClass="ddlsmall" ></asp:DropDownList>
                                              </td>
                                            </tr>
                                            <tr>
                                        ...
                                        <td class="v-top h-30">
                                            <table class="w-100p">
                                                <tr>
                                                    ...
                                                    <td class="a-center">
                                                         <asp:Label ID="lbldr" runat="server" Text="Select Approvel User"></asp:Label>
                    <asp:DropDownList ID="ddldoctor" runat="server" AutoPostBack="False" CssClass="ddlsmall" meta:resourcekey="ddldoctorResource1"> 
                                                        </asp:DropDownList> <asp:Button ID="btnSave" runat="server" CssClass="btn" OnClick="btnApproval_Click"
                ...
                <%--<ajc:ModalPopupExtender ID="ModalPopupExtenderDr" runat="server" BackgroundCssClass="modalBackground"
       DynamicServicePath="" Enabled="True" PopupControlID="pnlDoctor" CancelControlID="btnca"
 ...
 <%--   <div id="modal_dialog" style="display: none">
            ...
            <table class="a-center w-90p">
                <tr>
                    <td class="w-100p">
                   <asp:Label ID="lbldr" runat="server" Text="Select Approvel User"></asp:Label>
                    <asp:DropDownList ID="ddldoctor" runat="server" CssClass="ddlsmall" meta:resourcekey="ddldoctorResource1">
                                                        </asp:DropDownList>
                    </td>
                    </tr>
                    </table>
...
//    var checks = $(this).parent().find('#ChkAppr');
//    var allChecked = true;
    var dd = $(this).closest('tr').find('select[bulkapp="ddl"]')[0];
           var txtid=dd.name.replace('$ddlstatus','_txtValue');
        var txtval=document.getElementById(txtid).value;
        
    if ($(this).is(':checked')) {
    var textToFind = 'Approve';
...
for (var i = 0; i < dd.options.length; i++) {
    ...
    if(txtval.trim()=='')
    {
    alert("Investigation value is empty");
    }
    }
    else{
    var textToFind = 'Completed';
...
//        {
//         var ff=$(value).closest('tr').find('select[bulkapp="ddl"]');
//         var txtid=ff[0].name.replace('$ddlstatus','_txtValue');
//        var txtval=document.getElementById(txtid).value;
////         if(txtval!='')
////         {
...
$('input[name=ctl14]').change(function() {
    ...
    var allChecked = true;
    $.each(checks, function(idx, value) {
     var dd = $(value).closest('tr').find('select[bulkapp="ddl"]')[0];
        if (!$(this).is(':checked')) {
         
            allChecked = false;
//             var ff=$(value).closest('tr').find('select[bulkapp="ddl"]');
//    ff.prop('selectedIndex', 5);
           var txtid=dd.name.replace('$ddlstatus','_txtValue');
        var txtval=document.getElementById(txtid).value;
         if(txtval!='')
         {
...
//            var chkid=document.getElementById(ff[0].name.replace('$ddlstatus','_tdCheckbox')).children[0].id;
