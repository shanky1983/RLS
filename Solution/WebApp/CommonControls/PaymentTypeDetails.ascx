                ...
                <div class="dataheaderInvCtrl">
                    ...
                    <table class="tabletxt w-100p">
                        <tr class="a-left">
                            ...
                            <td nowrap="nowrap">
                                <asp:Label ID="Rs_Bank" Style="display: none" runat="server" Text="Bank/Card Type"
                                    ...
                                    <asp:Label ID="Rs_UPI" Style="display: none" runat="server" Text="Bank/Card Type/UPI"
                                    meta:resourceKey="Rs_UPIResource1"></asp:Label>
                            </td>
                            ...
                            <td nowrap="nowrap" id="tdlblCardHolderName" width="110px">
                                ...
                                <asp:Label ID="Rs_UPIRefNo" Style="display: none" runat="server" Text="Cheque/Card/DDNo."
                                    meta:resourcekey="Rs_Cheque_noResource1"></asp:Label>    
                            </td>
                        ...
                        <tr class="a-left">
                            ...
                            <td nowrap="nowrap">
                            <div class="autocomplete" style="width:200px;" id="autocompletediv">
                            <asp:TextBox runat="server" ID="txtUPITypeSearch" Style="display: none" CssClass="searchBox small" onkeyup="myFunction()"></asp:TextBox>
                                    </div>
                                ...
                                <cc1:AutoCompleteExtender ID="AutoCompleteProduct" runat="server" TargetControlID="txtBankType"
                                    ...
                                    ServiceMethod="GetBankName" ServicePath="~/OPIPBilling.asmx" OnClientItemSelected="SelectedItemValue"
                            ...
                            <td nowrap="nowrap">
                                <div id="tdtxtUnits" runat="server" style="display: none;">
                                    <asp:TextBox ID="txtUnits" runat="server"    onkeypress="return ValidateOnlyNumeric(this);" onblur="javascript:SetAmountbasedoncoupon()"  
...
<script type="text/javascript" language="javascript">
    var EMIValue = 0;
    var couponvalueandcount = ""; //couponamount/counponorderedunits/couponconsumedunits
    var iseditflag = "";
//arun coupon enabled
    function SetAmountbasedoncoupon() {
       // debugger;
        var couponarry = couponvalueandcount.split("~");
        if (couponarry != null && couponarry != undefined && couponarry.length >0) {
            var couponvalue = couponarry[0];
            var orderedunits = couponarry[1];
            var consumedunits = couponarry[2];
            var remaingavailunits = parseInt(parseInt(orderedunits) - parseInt(consumedunits));
            var unitsenter = document.getElementById('<%= txtUnits.ClientID %>').value;
            ...
            var greaterunitsentered = "Units get beyond the ordered limit";
            if (parseInt(remaingavailunits) < parseInt(unitsenter)) {
                ValidationWindow(greaterunitsentered, AlertType);
                document.getElementById('<%= txtUnits.ClientID %>').value = "";
                return false;
            }
            var amountforunits = parseFloat(parseFloat(couponvalue) * parseInt(unitsenter));
            document.getElementById('<%= txtAmount.ClientID %>').value = format_number(amountforunits, 2);
            document.getElementById('<%= txtTotalAmount.ClientID %>').innerHTML = format_number(amountforunits, 2);
    ...
    function SelectedItemValue(source, eventArgs) {
        ...
        var paymenttypevalue = document.getElementById("<%= ddlPaymentType.ClientID %>").value; //check paymenttype as coupon
        document.getElementById("<%= txtNumber.ClientID %>").value = arrGetVal[2];
        document.getElementById('<%=hdnReferenceID.ClientID %>').value = arrGetVal[0];
        if (arrGetVal != null && arrGetVal != undefined && arrGetVal.length > 3)
            couponvalueandcount = arrGetVal[3] + '~' + arrGetVal[4] + '~' + arrGetVal[5];
       // debugger;
        var expirydate = arrGetVal[6];
        var expirydatearry = expirydate.split("/");
        var expiryyear = expirydatearry[2];
        var expirymonth = expirydatearry[1];
        var expirydate = expirydatearry[0];
        var today = new Date();
        var todayyear = today.getFullYear();
        var todaymonth = (today.getMonth() + 1);
        var todaydate = today.getDate();
        if (parseInt(expiryyear) < parseInt(todayyear)) {
            alert("Coupon got expired");
        ...
        else if (parseInt(expirymonth) < parseInt(todaymonth) && (parseInt(expiryyear) == parseInt(todayyear))) {
            alert("Coupon got expired");
            document.getElementById('<%= txtBankType.ClientID %>').value = "";
            document.getElementById('<%= txtBankType.ClientID %>').focus();
            document.getElementById('<%= txtNumber.ClientID %>').value = "";
        ...
        else if (parseInt(expirydate) < parseInt(todaydate) && (parseInt(expirymonth) == parseInt(todaymonth)) && (parseInt(expiryyear) == parseInt(todayyear))) {
            alert("Coupon got expired");
            document.getElementById('<%= txtBankType.ClientID %>').value = "";
            document.getElementById('<%= txtBankType.ClientID %>').focus();
            document.getElementById('<%= txtNumber.ClientID %>').value = "";
    ...
    function PaymentSaveValidationQuickBill() {
        ...
        var PaymentBankType =PaymentTypeID == "12" ? document.getElementById('<%= txtUPITypeSearch.ClientID %>').value : document.getElementById('<%= txtBankType.ClientID %>').value;
    ...
    function PaymentTypeValidation() {
        ...
        //change by arun - UPI option added
        if (PaymentTypeID == "12" && PaymentBankType == "") {            
                alert("Please Enter the Bank/Card Type/UPI");
                return false;
            }
           else if (PaymentTypeID == "12" && CardHolderName == "") {
                alert("Please Enter the Cheque/Card/DDNo.");
                return false;
            }        
        //
        ...
        //change by arun - UPI option added
        else if (PaymentTypeID != "1" && PaymentMethodNumber == "" && PaymentTypeID != "12") {
        ...
        //else if (PaymentBankType == "" && PaymentName != "Cash") {
            ...
            if (AlreadyPaymentModeValidation != "") {
                ...
                for (j = 0; j < x.length; j++) {
                    if (x[j] != "") {
                        var val = x[j].split("~");
                        //changes by arun - if we select upi option after cash payment type, for both payment type have PaymentBankType is empty
                        if ((val[6].split("^")[1] == PaymentMethodNumber && PaymentMethodNumber != "") && (val[7].split("^")[1] != PaymentBankType && PaymentBankType != "")) {
            ...
            if (PaymentTypeID != "1" && PaymentTypeID != "12") {                
                ServiceCharge = format_number(ServiceCharge, 2);
            }
            ...
            if (arrayPaymentMainData.length > 0) {
                ...
                for (iarrayPMainDataCount = 0; iarrayPMainDataCount < arrayPaymentMainData.length - 1; iarrayPMainDataCount++) {
                    ...
                    if (PaymentAmount != 0) {
                        ...
                        newPaymentTables += "<TR><TD >" + PaymentName + "</TD>";
                        newPaymentTables += "<TD >" + PaymentAmount + "</TD>";
                        //changes by arun - UPI added
                        if (PaymentTypeID == "12") {
                            newPaymentTables += "<TD >" + CardHolderName + "</TD>";
                        }
                        else {
                            newPaymentTables += "<TD >" + PaymentMethodNumber + "</TD>";
                        }
...
//                        newPaymentTables += "<TD >" + PaymentMethodNumber + "</TD>";
                        ...
                        if (PaymentTypeID == "12") {
                            var upiref = "";
                            newPaymentTables += "<TD style='Display:block;'>" + upiref + "</TD>";
                        }
                        else {
                            newPaymentTables += "<TD style='Display:block;'>" + CardHolderName + "</TD>";
                        }
//                        newPaymentTables += "<TD style='Display:block;'>" + CardHolderName + "</TD>";
    ...
    function btnPaymentEdit_OnClick(sEditedData) {
        ...
        if (arrayRemarks.length > 0) {
            var paymenttypeid = arrayPaymentName[1];
            if (paymenttypeid != null && paymenttypeid != undefined && paymenttypeid == "UPI Payments") {
                document.getElementById('<%= txtUPITypeSearch.ClientID %>').value = arrayBankName[1];
            }
            else{
    ...
    function Paymentchanged() {
        ...
        document.getElementById('<%= txtUnits.ClientID %>').value = "";
        document.getElementById('<%= txtCardHolderName.ClientID %>').value = "";
        document.getElementById('<%= txtUPITypeSearch.ClientID %>').value = "";
        //
        var slist = { ChequeCardDDNo: '<%= Resources.ClientSideDisplayTexts.CommonControls_PaymentTypeDetails_ChequeCardDDNo_1 %>', BankCardType: '<%= Resources.ClientSideDisplayTexts.CommonControls_PaymentTypeDetails_BankCardType_1 %>', CouponNumber: '<%= Resources.ClientSideDisplayTexts.CommonControls_PaymentTypeDetails_CouponNumber %>', Coupon: '<%= Resources.ClientSideDisplayTexts.CommonControls_PaymentTypeDetails_Coupon %>', CouponName: '<%= Resources.ClientSideDisplayTexts.CommonControls_PaymentTypeDetails_CouponName %>' };
        var IsEMI = 'N';
        var IsEMIConfig = document.getElementById('<%=hdnIsEMI.ClientID %>').value; //get Config Value isEMi or Not 
        var IsnneedPrepaidCard = document.getElementById('<%=hdnIsneedPrepaid.ClientID %>').value; 
        ...
        document.getElementById('<%=Rs_UPI.ClientID %>').style.display = 'none';
        document.getElementById('<%=Rs_UPIRefNo.ClientID %>').style.display = 'none';
        document.getElementById('<%= txtBankType.ClientID %>').style.display = 'none';
        document.getElementById('<%= txtUPITypeSearch.ClientID %>').style.display = 'none';
       //
	   //arun coupon enabled//when edit the payment details units get empty
        if (iseditflag != "1" || sPaymentType != "Coupon") {
            document.getElementById('<%= txtUnits.ClientID %>').value = "";
        }
        document.getElementById('<%=lblUnits.ClientID %>').style.display = 'none';
        document.getElementById('<%=tdtxtUnits.ClientID %>').style.display = 'none';
       ...
       // else if (sPaymentType == "Coupon") {
		...
		//arun coupon enabled
		document.getElementById('<%=Rs_CardHolderName.ClientID %>').style.display = 'none';
        document.getElementById('<%=txtCardHolderName.ClientID %>').style.display = 'none';
        document.getElementById('<%=tdCardHolder.ClientID %>').style.display = 'none';
        document.getElementById('<%=txtCardHolderName.ClientID %>').value = "";
        document.getElementById('<%=Rs_Cheque_no.ClientID %>').style.display = 'Block';
        document.getElementById('<%=Rs_Bank.ClientID %>').style.display = 'Block';
            //
        document.getElementById('<%=Rs_Cheque_no.ClientID %>').innerHTML = slist.CouponNumber;
        document.getElementById('<%=Rs_Bank.ClientID %>').innerHTML = slist.CouponName;
        document.getElementById('<%=txtNumber.ClientID %>').style.display = 'Block';
        document.getElementById('<%=lblChequeDate.ClientID %>').style.display = 'none';
        document.getElementById('<%=tdtxtDate.ClientID %>').style.display = 'none';
        document.getElementById('<%=txtBankType.ClientID %>').style.display = 'Block';
        document.getElementById('<%=lblUnits.ClientID %>').style.display = 'block';
        document.getElementById('<%=tdtxtUnits.ClientID %>').style.display = 'block';
        document.getElementById('<%=txtServiceCharge.ClientID %>').style.display = 'none';
        document.getElementById('<%=Rs_Ser_Cha.ClientID %>').style.display = 'none';
        document.getElementById('<%=hdnReferenceType.ClientID %>').value = slist.Coupon;
            var sval;
            sval = '0' + '~' + sPaymentType;
         ...
         if (sPaymentTypeId == "11") {
           ...
           // if (sPaymentType == "Credit/Debit Card" && sPaymentType != "Cheque") {
            if (sPaymentTypeId == "3" && sPaymentTypeId != "2") {
			    document.getElementById('<%= txtCardHolderName.ClientID %>').style.display = 'block';
           ...
           // if (sPaymentType == "Credit/Debit Card" && sPaymentType != "Cheque") {
            ...
            //debugger
                //changes by arun - UPI added
            if (sPaymentTypeId == "12") {
                document.getElementById('autocompletediv').style.display = 'Block';
                document.getElementById('<%= txtBankType.ClientID %>').style.display = 'none';
                document.getElementById('<%= txtUPITypeSearch.ClientID %>').style.display = 'Block';
                //document.getElementById('<%= txtUPITypeSearch.ClientID %>').value = "";
                document.getElementById('<%= txtUPITypeSearch.ClientID %>').focus();
                document.getElementById('<%=Rs_Bank.ClientID %>').innerHTML = slist.BankCardType;
                document.getElementById('<%=txtNumber.ClientID %>').style.display = 'none';
                document.getElementById('<%=txtServiceCharge.ClientID %>').style.display = 'none';
                document.getElementById('<%=Rs_Ser_Cha.ClientID %>').style.display = 'none';
                ...
                //changes
                document.getElementById('<%=txtCardHolderName.ClientID %>').style.display = 'Block';
                document.getElementById('<%=Rs_UPI.ClientID %>').style.display = 'Block';
                document.getElementById('<%=Rs_Bank.ClientID %>').style.display = 'none';
                document.getElementById('<%=Rs_UPIRefNo.ClientID %>').style.display = 'Block';
                document.getElementById('<%=Rs_UPIRefNo.ClientID %>').innerHTML = slist.ChequeCardDDNo;
                document.getElementById('<%= Rs_Cheque_no.ClientID %>').style.display = 'none';
...
//            }
        ...
        if(sPaymentTypeId == 4 && IsnneedPrepaidCard == 'Y')
        {
          document.getElementById('<%= txtServiceCharge.ClientID %>').style.display = 'none';
    ...
    //changes by arun - UPI Type List auto complete
    //debugger;
    var upilist = ["PhonePe", "Paytm", "BHIM app", "MobiKwik", "Google Tez", "Uber", "Chillr", "Paytm Payments Bank", "SBI Pay", "iMobile","Axis Pay","BOB UPI"];
    function myFunction() {
        //debugger;
        autocomplete(document.getElementById('<%= txtUPITypeSearch.ClientID %>'), upilist);
    }
...
//
    ...
    function changeAmountValues() {
        ...
        //changes by arun - UPI added
        sServiceAmount = isNaN(sServiceAmount) ? "0" : sServiceAmount;
...
<asp:HiddenField ID="hdnIsneedPrepaid" runat="server" Value="N" />
