...
//arun changes
setTimeout(function() {    
        var alertpop = $('#<%=hdnAlertPopup.ClientID%>').val();
        var popupcount = $('#<%=hdnSessionPopupOpen.ClientID%>').val();
        if (alertpop == "1" && popupcount == "1") {            
            $('.popup').toggleClass("open");
            document.getElementById("<%=PopupText.ClientID%>").style.display = "block";
        }
}, 300);
$(document).ready(function() {
    $('#PopupBand').click(function() {                      
        document.getElementById("<%=PopupText.ClientID%>").style.display = "none";
    });
});
</script>
<!-- Preloader -->
...
<div id="header">
	...
	<asp:HiddenField ID="hdnAlertPopup" runat="server" />
	<asp:HiddenField ID="hdnSessionPopupOpen" runat="server" />
	<div class="popup" id="PopupText" runat="server" style="display: none">
			<div class="band" id="PopupBand">
			 <label class="war">Warning!</label>
			 <label class="tt">X</label>
		
			</div>
			<div class="content" id="PopUplbl" runat="server">hellow world.</div>
		</div>
                        </div>
                        <div class="topNav">
                            <ul>
