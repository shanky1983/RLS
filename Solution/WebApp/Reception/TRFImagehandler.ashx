...
public class TRFImagehandler : IHttpHandler
{
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            string HCTRFImages = GetConfigValue("IsHCTRFImages", Convert.ToInt16(context.Request.QueryString["OrgID"]));
				...
				CLogger.LogWarning("filePath" + filePath);
                    ...
                    else
                    {
                        if(HCTRFImages == "Y")
                        {
                            ...
                            CLogger.LogWarning("HC TRFImages filePath -" + filePath);
                            if (File.Exists(filePath))
                            {
                                CLogger.LogWarning("Exists HC TRFImages filePath -" + filePath);
                                if (imageExtension.Contains(fileExtension.ToUpper()))
                                {
                                    Image image = Image.FromFile(filePath);
                                    context.Response.ContentType = "Image/jpeg";
