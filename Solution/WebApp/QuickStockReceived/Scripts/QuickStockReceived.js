...
if ($('#hdnIsVATNotApplicable').val().trim() != "Y")
    ...
    cell11.innerHTML = slist.Nominal;
    cell12.innerHTML = slist.Discount;
    cell13.innerHTML =  $('#hdnIsVATNotApplicable').val().trim() == "Y" ? "VAT":slist.Tax;
    cell14.innerHTML = slist.SellingPrice;
    cell15.innerHTML = slist.Ex_percent;
    cell16.innerHTML = slist.MRP;
    cell17.innerHTML = slist.RakNo;
         ...
         cell20.setAttribute('colspan',4)
            ...
            if ($('#CheckState').val() == "Y") {
                if (value.Tax > 0) {
                        ...
                        //GSTtaxamount = '0.0';//(parseFloat(parseFloat(ptotal) * value.Tax / 100)).toFixed(2);
                        //GSTTax = '0.0';//(parseFloat(value.Tax)).toFixed(2);
                        //$('#hdnTax').val(GSTTax);
                        //ToTargetFormat($('#hdnTax'));
                        cell15.innerHTML = '0.0';//ToTargetFormat($('#hdnTax')); //GSTTax;  // Tax
                        //$('#hdnTotalTax').val(GSTtaxamount);
                        //ToTargetFormat($('#hdnTotalTax'));
                        cell16.innerHTML = ToTargetFormat($('#hdnTotalTax')); //GSTtaxamount;
...
//            var SubDiscount = parseFloat(parseFloat(parseFloat(value.Discount) / 100) * parseFloat(parseFloat(value.UnitPrice) * parseFloat(value.RECQuantity))).toFixed(2);
            ...
            if ($('#hdnIsVATNotApplicable').val().trim() == "Y") {
                ...
                //cell13.hidden=true;
                //cell14.hidden=true;
        //        cell14.setAttribute('colspan', 3) //RM01
                //cell19.hidden = true;
                //cell20.hidden = true;
               // cell21.hidden = true;
                //cell18.hidden = true;
    ...
    $('#tblPODetail').removeClass().addClass('w-100p animated fadeIn');
    //CSTCalculation();
    if($("#hdnIsTCSSupplier").val()=="Y" && $("#hdnNeedTcsTax").val()=="Y")
    {
    
        $('#tblTCS').removeClass().addClass('displaytr');
        var GTotal = parseFloat(parseFloat(ToInternalFormat($("#txtGrandTotal"))));
        var PODiscount = parseFloat(parseFloat(ToInternalFormat($("#txtTotalDiscount"))));
        var TCsPer =parseFloat(parseFloat(ToInternalFormat($("#hdnTcsTaxPer"))));
        var TCSValue = (( GTotal + PODiscount )*TCsPer)/100;
        var totalTcs = GTotal +TCSValue;        
        $("#txtlblTCS").val( TCSValue );
        $("#txtGrandTotal").val( totalTcs );
    }
    else{
     $("#txtlblTCS").val( '0.00' );
    }
