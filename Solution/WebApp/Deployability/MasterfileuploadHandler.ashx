...
public class MasterfileuploadHandler : BasePage, IHttpHandler, IReadOnlySessionState
{
    ...
    //arun changes
    string shideLabShareAmnt = string.Empty;  
    public void ProcessRequest(HttpContext context)
    {
        if (context.Request.Files.Count > 0)
        {
            ...
            //arun changes
            if (context.Request.QueryString["HideLabShareAmnt"] != null)
            {
                shideLabShareAmnt = context.Request.QueryString["HideLabShareAmnt"].ToString();
                shideLabShareAmnt = shideLabShareAmnt.Trim();
            }
            //
            ...
            for (int i = 0; i < context.Request.Files.Count; i++)
            {
                ...
                {
                    if (shideLabShareAmnt == "Y" && type.Equals("RateMaster"))
                    {
                        file_Name = "RateMaster_Franchisee";
                    }
                    else
                    {
    ...
    private System.Data.DataTable ReadExcelxlsformat(string fileName, string fileExt, string mastertype)
    {
		...
		conn = @"provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + fileName + ";Extended Properties='Excel 8.0;HDR=Yes;IMEX=1';"; //for below excel 2007 
        ...
        using (OleDbConnection con = new OleDbConnection(conn))
        {
            try
            {
                ...
                if (mastertype.Equals("RateMaster"))
                {
                    ...
                    if (shideLabShareAmnt == "Y")
                    {
                        dtexcel.Columns[8].DataType = typeof(decimal);
                        dtexcel.Columns[9].DataType = typeof(decimal);
                    }
    ...
    private System.Data.DataTable ReadExcel(string fileName, string fileExt, string mastertype)
    {
        ...
        else
        {
            //conn = @"provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + fileName + ";Extended Properties=HTML Import;Persist Security Info=False;";
 			conn = @"provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + fileName + ";Extended Properties=HTML Import;Persist Security Info=False;";
        }
        String sTableName = string.Empty;
        using (OleDbConnection con = new OleDbConnection(conn))
        {
    ...
    private DataTable HeaderCheckingGetDataFromExcel(string filetype, string mastertype, string csvfilepath)
    {
        try
        {
            ...
            if (mastertype.Equals("RateMaster"))
            {
                if (shideLabShareAmnt == "Y")
                {
                    fileNamess = "RateMaster_Franchisee";
                }
                else
                {
        ...
        try
        {
            using (TextFieldParser csvReader = new TextFieldParser(csv_file_path))
            {
                ...
                if (type.Equals("RateMaster"))
                {
                    ...
                    if (shideLabShareAmnt == "Y")
                    {
                        csvData.Columns[8].DataType = typeof(decimal);
                        csvData.Columns[9].DataType = typeof(decimal);
                    }
